 ...
 * function). Deliberately does NOT create any business data: no demo users,
 * no demo products, no demo orders/payments. The application must start and
 * behave correctly with empty products/categories tables (see spec section
 * "Database data policy" / "empty database behavior").
 *
 * To try the admin area: register a normal account through the app, then
 * manually add the ADMIN role for that user, e.g.:
